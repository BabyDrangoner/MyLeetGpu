...
- 提供简体中文的桌面端 Web 界面，浏览至少三道原创 CUDA 题目：Vector Addition、Matrix Transpose 和 Reduction。
- 使用 Monaco Editor 编辑规定的 `solve` 接口，支持 starter code、重置以及浏览器草稿自动保存。
- 严格区分“编译”“运行公开样例”“完整验证”和“保存为性能版本”。
- 显示经过清理和限长的 NVCC、运行错误、错误答案、超时及 stdout/stderr 诊断。
- 正确性通过后，由平台 harness 使用固定协议测量 GPU kernel 性能。
- 仅在用户显式保存且验证、benchmark 均成功后，原子地持久化不可变代码版本和 benchmark。
- 比较同一题目的多个版本，展示环境一致性、逐规模性能和相对 baseline 的 speedup。
...
- MVP 固定使用 `nvidia/cuda:12.4.1-devel-ubuntu22.04`。Worker 启动探测时读取镜像 RepoDigest，并把实际 digest 连同 GPU/CUDA/NVCC 信息写入环境快照。

### 3.3 前端状态与可访问性

列表、题目、草稿、环境和版本请求都必须分别呈现 loading、empty、error 与 retry 状态，不能用空白区域代替失败信息。任务面板持续显示 Job ID、当前阶段、进度和终态；timeout 与平台错误不得伪装成普通错误答案。危险操作用可聚焦的模态框确认，键盘可完成编辑与提交，状态不能只用颜色表达。桌面布局优先，同时在较窄窗口保持题面、编辑器和输出面板可访问。
...
| `backend/myleetgpu/runner` | Docker Runner adapter、资源限制、输出清理、超时、容器清理、GPU 探测 | Runner 端口和受控配置 | 接收任意命令、编译参数、路径或 shell 字符串 |
...
| `problems` | 题面、starter、接口头文件、可信 validator/benchmark harness 和测试声明 | 题目 schema | 将题目特例写进 Judge 核心 |
...
│   ├── validator.cu
│   └── benchmark.cu
└── tests/                 # 可选；当前三题的用例配置在 problem.yaml
```

用户只能实现 `solve.h` 约定的接口。入口 `main`、数据生成、拷贝、同步、参考实现、结果验证和计时均由可信平台 harness 控制。
...
- 从平台 allowlist 中选择的编译配置，不能包含任意用户 flags。
...
当前 `suite_hash` 精确覆盖 `benchmark.cu`、公开接口 `solve.h` 和规范化后的 `benchmark` manifest（其中含输入规模、种子、warmup、iterations 和协议版本）。validator、公开/内部正确性配置的变化通过 problem revision 表达；修改任何影响正确性语义的内容必须增加 revision，修改性能口径还会自然改变 suite hash。新增符合协议的目录不需要修改核心 Judge。
...
- `id`、`problem_id`；单用户模式下每题至多一个活动草稿；
- 当前源码和更新时间。

草稿可变，不是性能版本。当前服务端采用按题目的 upsert/后写覆盖；浏览器同时使用本地草稿作为网络失败时的回退。自动保存、重置、编译、运行和验证均不得创建 `Version`。

### 6.2 `Version`

- `id`、`problem_id`、`problem_revision`；
- 用户可修改的名称和备注；
...
版本的源码、题目 revision 和初次测量语义不可修改。重命名或改备注只更新元数据。删除版本必须在 UI 二次确认，API 还要求 `confirmed=true`，并在事务中级联删除关联 benchmark。前端通过 duplicate 查询提示相同 source hash；服务端也在入队、Worker 开始和最终提交前复查，只有显式 `allow_duplicate=true` 才允许继续。

### 6.3 `Job`

- `id`（也是全链路 correlation ID）、动作类型、题目与 revision；
...
- Windows/WSL 可见的驱动版本、容器 CUDA Runtime 和 NVCC 版本；
- 固定 CUDA 镜像引用及实际 image digest；
- 集中探测所得的 CUDA 架构；
...
稳定的工具链和硬件字段经规范化后计算 `environment_fingerprint`。温度、瞬时时钟、功耗和 GPU busy 等易变遥测只随快照记录，不参与 fingerprint，否则同一机器的连续运行会被错误判为不同环境。不能用空字符串或伪造值补齐 WSL 无法提供的遥测。
...
| `GET /api/ready` | 检查数据库、题目、健康环境快照、Worker 活跃租约和熔断文件；不就绪时返回 503 |
| `GET /api/environment` | 最近一次 GPU/CUDA/Runner 环境快照；熔断状态以 `/api/ready` 为准 |
| `GET /api/problems` | 题目摘要列表 |
| `GET /api/problems/{slug}` | 题面、签名、starter、限制和 revision；不返回内部测试 |
| `GET /api/drafts/{problem_id}` | 获取该题草稿 |
| `PUT /api/drafts/{problem_id}` | 按题目 upsert 草稿；当前为后写覆盖 |
| `POST /api/jobs` | 创建 `compile`、`run`、`validate`、`save_version` 或 `rebenchmark` Job；返回 202 和 Job ID |
| `GET /api/jobs/{job_id}` | 获取状态、阶段、逐公开用例结果和受限诊断 |
| `GET /api/problems/{problem_id}/versions` | 列出手动保存版本和 benchmark 摘要 |
| `PATCH /api/versions/{version_id}` | 仅修改名称和备注 |
| `DELETE /api/versions/{version_id}?confirmed=true` | UI 二次确认后删除；缺少/否定确认参数返回 409 |
| `GET /api/versions/duplicates?problem_id=...&source_hash=...` | 保存前查询重复源码并提示用户 |
| `POST /api/versions/compare` | 比较同题 2 至 8 个唯一版本并指定其中一个 baseline |
| `GET /api/docs`、`GET /api/openapi.json` | 本地交互式 API 文档和 OpenAPI schema |

保存版本和重测都通过 `POST /api/jobs` 进入同一状态机：`save_version` 请求携带名称、备注、点击时源码和 `allow_duplicate`。服务在入队、Worker 开始及最终提交前检查重复；发现重复且未显式允许时返回/记录冲突，`allow_duplicate=true` 才允许继续。`rebenchmark` 请求携带同题的 1 至 8 个唯一版本 ID。MVP 没有取消路由；状态枚举保留 `cancelled` 供受控关停或后续取消协议使用，浏览器断开不等同于取消 Job。
...
1. API 校验题目 revision、动作、源码字节数和可选元数据。
...
5. Runner 根据动作选择固定镜像、固定命令模板和 allowlist 配置。
6. 无论成功、失败、取消或超时，均执行容器及临时目录清理。

### 9.2 编译

```text
源码快照 → 无 GPU 编译容器 → NVCC → 清理/截断诊断 → 清理产物 → Job 终态
```

编译容器不获得 GPU。只生成临时产物，不运行二进制，不创建 Version 或 BenchmarkRun。NVCC 输出会删除宿主机路径、spool 路径和内部 harness 路径，保留用户文件名、行列号和关键诊断；当前 Runner 合并捕获 stdout/stderr，并对合并后的字节总量设限、标注超限或截断。
...
只有编译成功后才创建 GPU 容器。返回每个公开用例的 pass/fail、公开输入摘要、错误类别和必要输出；不创建版本。容器初始化、用例准备等不作为可持久化性能成绩。
...
sequenceDiagram
    ...
    R-->>W: 原始样本
    W->>R: 强制环境探测
    R-->>W: GPU/CUDA/NVCC + image digest
...
- 仅平台 `benchmark.cu` harness 的时间有效；完全忽略用户打印的时间。
- benchmark 前必须使用同一源码快照重新完成完整正确性验证。
- 单 GPU 串行执行，benchmark 之间不得重叠。
- 编译、容器创建、CUDA context 初始化、输入生成、设备内存分配和 H2D/D2H 拷贝均在计时区间外。
...
- 同一规范化编译配置；
- 同一 environment fingerprint（包括 GPU/Compute Capability、驱动、CUDA/NVCC、Runner 镜像 digest 等）。

任何字段不一致时，UI 必须明确标为“不可直接比较”，列出差异，不能生成统一排名或具有误导性的总 speedup。可以并排展示历史原始数据。用户可选择“使用当前统一环境重新测试所选版本”；系统先串行验证全部所选版本，再串行 benchmark 全部版本，并按 10.4 节批量追加新的 BenchmarkRun，不改变源码版本。

比较页对每个输入规模展示 median、p95、CV/MAD、样本数和相对 baseline 的 speedup，并同时展示 GPU、驱动、CUDA Runtime、NVCC、编译 flags、镜像 digest、suite hash 与协议版本。源码快照应只读展示，优先用 Monaco Diff 做两两差异查看；超过两个版本时由用户选择 diff 的左右两侧。
...
`rebenchmark` 接受同题的 1 至 8 个唯一版本，并拒绝已经不是当前 problem revision 的版本。Worker 先探测并保存统一环境快照，然后严格分成两阶段：
...
攻击者可提交任意能被 NVCC 接受的 CUDA/C++ 源码，包括无限循环、fork bomb、大量输出、文件探测、网络访问、资源耗尽以及触发 GPU 异常的 kernel。需要保护仓库、数据库、Docker daemon、主机普通文件和服务可用性，并避免内部测试泄漏。
...
- 固定 `nvidia/cuda:12.4.1-devel-ubuntu22.04` 并在启动探测中记录实际 RepoDigest，禁止 `latest`；
- 非 root UID/GID，`--network none`，只读 root filesystem；
- 仅提供 `/tmp` 的 64 MiB、`nosuid,nodev,noexec` tmpfs，禁止写入镜像其他位置；
...
| 阶段 | 精确 bind mount | 写权限 | GPU |
| --- | --- | --- | --- |
| 编译 | 当前 Job 下的 `compile-validator/` 或 `compile-benchmark/`，仅含规范化 `source.cu`、`solve.h`、选定的 `platform.cu`，以及本阶段生成的 `program` | `/work` 可写，以便非 root NVCC 生成临时目标和可执行文件 | 无，不传 `--gpus` |
| 公开运行/完整验证/benchmark | 新建的 `run-public/`、`run-full/` 或 `run-benchmark/`，只复制最终 `program` | bind mount 为 readonly，目录和程序只读/可执行 | 仅 `--gpus device=0` |
...
编译阶段需要平台 harness 完成链接，但运行容器只接收最终二进制，不携带源码、题目仓库或独立测试文件。对浏览器的响应和普通日志还会过滤内部测试输入、参考输出及真实 harness/Job 路径。由于本项目面向能直接读取本地仓库的可信操作者，“内部测试不泄漏”是产品输出边界，而不是针对本机管理员的保密承诺。
...
| 类别 | 示例 | 用户可见行为 |
| --- | --- | --- |
| `compile_error` | NVCC 语法、类型或链接错误 | 行列号和清理后的诊断 |
| `wrong_answer` | 值不匹配、非法 NaN/Inf | 公开测试可显示有限详情；内部测试只显示安全摘要 |
| `runtime_error` | 非零退出、CUDA error、非法访问 | 稳定错误码、退出信息和限长合并输出 |
| `output_limit` | 合并 stdout/stderr 超限 | 终止任务并明确标注输出超限 |
| `timeout` | 任一阶段超过 wall time | Job 进入 `timed_out`，终止容器并清理 |
| `runner_unhealthy` | GPU/驱动/容器探测或运行链路异常 | Job 进入 `system_error`，提示恢复 Runner |
...
- 用户代码永远不参与构造 shell 字符串、路径或任意 NVCC flags。
- 编译容器挂载精确可写编译目录且无 GPU；执行容器只读挂载仅含可执行文件的精确目录，并只获得 GPU 0。两类用户容器均无网络、无 Docker socket、无仓库/数据库挂载，且保留 Docker 默认内置 seccomp profile。
