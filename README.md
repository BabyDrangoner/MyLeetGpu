...
MyLeetGpu 是一个面向单机可信操作者的 GPU 编程、正确性验证与性能比较环境。它把 CUDA C++、Triton (Python) Kernel 和高层 PyTorch (Python) 作为三种一等实现类型；当前内置六道 CUDA/Triton Kernel 题，以及多头自注意力（MHA）和分组查询自注意力（GQA）两道 PyTorch 题。平台提供 Monaco 编辑器、异步隔离 Judge、手动性能版本、持久化 benchmark 和同语言统一口径比较。
...
Vector Addition、Matrix Transpose、Sum Reduction、Max Reduction、Softmax 和 Matrix Multiplication 支持在编辑器顶部切换 `CUDA C++` 与 `Triton (Python)`；MHA 和 GQA 使用 `PyTorch (Python)`，要求实现保存固定投影权重的自注意力 class，唯一前向输入为 `X` 与 `isCasual`。URL、服务端草稿、本地回退草稿和性能版本都带实现语言；切换语言不会覆盖另一套源码。
