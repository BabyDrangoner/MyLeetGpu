...
Measurement run_benchmark(const BenchmarkCase& benchmark_case,
    ...
    DeviceBuffer<float> device_a(count);
    DeviceBuffer<float> device_b(count);
    ...
    for (int i = 0; i < kWarmup * benchmark_case.inner_repetitions; ++i) {
        solve(device_a.get(), device_b.get(), device_output.get(),
              benchmark_case.n, stream);
    }
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        if (std::isnan(output[i]) ||
            error > 1.0e-6F + 1.0e-6F * std::fabs(expected)) {
