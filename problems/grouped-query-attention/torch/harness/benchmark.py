...
RTOL = 2.0e-4
CASES = (
    ("B1-QH8-KVH2-Q128-K128-D64", 1, 8, 2, 128, 128, 64, "all", 8),
    ("B2-QH16-KVH4-Q384-K512-D64", 2, 16, 4, 384, 512, 64, "prefix", 1),
    ("B1-QH32-KVH8-Q1024-K1024-D64", 1, 32, 8, 1024, 1024, 64, "causal", 1),
)
...
def run_benchmark(
    ...
    query = (
        torch.randn(
            (batch, query_heads, query_length, head_dim), generator=generator, dtype=torch.float32
        )
        * 0.5
    )
    key = (
        torch.randn(
            (batch, key_value_heads, key_length, head_dim),
            generator=generator,
            dtype=torch.float32,
        )
        * 0.5
    ...
    ensure_inputs_unchanged(inputs, snapshots)
    return {
        "size": batch * query_heads * query_length * key_length * head_dim,
        "label": label,
        "samples_ms": samples,
